<?php
	...
	public function getAllStepProgressByFacility($id){
		...
		$totalMetricsA = (($this->db->select('*')->from('tbl_form_input i')->join('tbl_metric m', 'i.metric_id=m.metric_id')->where(array('i.step_id <' => '14','i.facility_id' => $id, 'm.metric_disabled' => 0, 'i.metricstep_status' => 1))->get()->num_rows()) / $totalMetricsCount*100);
		...
		$totalMetricsAA = (($this->db->select('*')->from('tbl_form_input i')->join('tbl_metric m', 'i.metric_id=m.metric_id')->where(array('i.step_id < ' => '27', 'i.step_id > ' => '13','i.facility_id' => $id, 'm.metric_disabled' => 0, 'i.metricstep_status' => 1))->get()->num_rows()) / ($totalMetricsCount)*100);
